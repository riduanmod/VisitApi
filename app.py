...
def index():
    # ডোমেইন ভিজিট করলে এই গাইডলাইন শো করবে (আপনার দেওয়া সিরিয়াল অনুযায়ী)
    guide_data = {
    ...
    # sort_keys=False দেওয়ার কারণে আপনি যেভাবে সিরিয়াল দিয়েছেন ঠিক সেভাবেই শো করবে
...
TOKEN_CACHE_DIR = "token_data"
TOKEN_CACHE_FILE = os.path.join(TOKEN_CACHE_DIR, "tokens_cache.json")
...
class AccountManager:
    ...
    def load_token_cache_from_file(self):
        ...
        if os.path.exists(TOKEN_CACHE_FILE):
            try:
                ...
                print(f"[+] Loaded existing tokens from {TOKEN_CACHE_FILE}")
            except Exception as e:
    ...
    def generate_all_tokens_concurrently(self, server_name="BD"):
        """Generates tokens for all accounts concurrently to save time."""
        uids = self.server_lists.get(server_name, [])
        if not uids:
            return

        print(f"[*] Generating tokens for {len(uids)} accounts... Please wait.")
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            futures = [executor.submit(self.generate_token_for_uid, uid, server_name) for uid in uids]
            concurrent.futures.wait(futures)
            
        print(f"[+] Token generation complete for server: {server_name}")
...
# =========================================================
# 🔄 BACKGROUND TOKEN REFRESHER (Runs every 5 hours)
# =========================================================
def auto_token_refresher():
    """Continuously checks and refreshes tokens every 5 hours."""
    while True:
        time.sleep(60) 
        current_time = time.time()
        needs_refresh = False
        server_to_refresh = "BD" 
        
        with acc_manager.lock:
            for uid, data in acc_manager.accounts_cache.items():
                if data.get("token") and (current_time - data.get("token_time", 0) >= TOKEN_REFRESH_INTERVAL):
                    needs_refresh = True
                    break
        
        if needs_refresh:
            print("\n[!] 5 Hours passed. Old tokens expired. Auto-generating new tokens...")
            acc_manager.generate_all_tokens_concurrently(server_to_refresh)

threading.Thread(target=auto_token_refresher, daemon=True).start()
...
def send_visits(server, uid):
    ...
    if not tokens:
        print(f"[!] No valid tokens found for {server}. Generating now...")
        acc_manager.generate_all_tokens_concurrently(server)
        tokens = acc_manager.get_valid_tokens_batch(server)
    ...
    if player_info:
        # আপনি ঠিক যেভাবে সিরিয়াল দিয়েছেন, সেভাবেই response তৈরি করা হয়েছে
        response_data = {
            "Developer": "Riduanul Islam",
            "TelegramBot": "https://t.me/RiduanFFBot",
            "TelegramChannel": "https://t.me/RiduanOfficialBD",
        ...
        # sort_keys=False নিশ্চিত করে যে আপনার ডিকশনারির অর্ডার পরিবর্তন হবে না
...
if __name__ == "__main__":
    print("[*] Initializing Server & Token Checker...")
    acc_manager.load_accounts("BD") 
    
    initial_tokens = acc_manager.get_valid_tokens_batch("BD")
    if not initial_tokens:
        acc_manager.generate_all_tokens_concurrently("BD")
