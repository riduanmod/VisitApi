...
def index():
    guide_data = {
        ...
        "Message": "API is running at super high speed!",
        "API_Usage_Guide": {
            "API_Format": {"Send_Visits": "/[Server_Name]/[Player_UID]"},
            "Examples": {"BD_Server": "/BD/1234567890", "IND_Server": "/IND/9876543210"}
        }
    }
...
def perform_login(uid, password, region):
    try:
        ...
        url_login = f"https://{host}/MajorLogin"
        ...
        headers_login = {
            ...
            "Host": host, "X-GA": "v1 1", "ReleaseVersion": RELEASE_VERSION
        }
...
class AccountManager:
    ...
    def load_token_cache_from_file(self):
        if os.path.exists(TOKEN_CACHE_FILE):
            try:
                with open(TOKEN_CACHE_FILE, "r") as f: self.accounts_cache = json.load(f)
            except Exception: pass

    def save_token_cache_to_file(self):
        try:
            with open(TOKEN_CACHE_FILE, "w") as f: json.dump(self.accounts_cache, f, indent=4)
        except Exception: pass

    def load_accounts(self, server_name="BD"):
        fname = "Accounts.ind.json" if server_name == "IND" else "Accounts.br.json" if server_name in ["BR", "US", "SAC", "NA"] else "Accounts.bd.json"
        ...
        try:
            ...
            with open(file_path, "r") as f:
                ...
                for acc in data:
                    uid, pwd = str(acc.get("uid")), acc.get("password")
                    if uid and pwd:
                        if uid not in self.accounts_cache: self.accounts_cache[uid] = {"password": pwd, "token": None, "token_time": 0}
    ...
    def get_valid_tokens_batch(self, server_name):
        ...
        valid_tokens, expired_uids, current_time = [], [], time.time()
        for uid in uids:
            account = self.accounts_cache.get(uid)
            if account and account.get("token") and (current_time - account.get("token_time", 0) < TOKEN_REFRESH_INTERVAL):
        ...
        if expired_uids:
            ...
            for uid in expired_uids:
                ...
                if acc and acc.get("token"): valid_tokens.append(acc["token"])
...
# =========================================================
# ULTRA FAST VISIT LOGIC
# =========================================================
def get_url(server_name):
    if server_name == "IND": return "https://client.ind.freefiremobile.com/GetPlayerPersonalShow"
    elif server_name in {"BR", "US", "SAC", "NA"}: return "https://client.us.freefiremobile.com/GetPlayerPersonalShow"
...
async def process_visits(tokens, uid, server_name, target_requests):
    ...
    # স্পিড বাড়াতে কনকারেন্সি ৩০০০ করা হয়েছে
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        async def sem_visit(token, need_info):
            async with sem:
                headers = {
                    "ReleaseVersion": RELEASE_VERSION, "X-GA": "v1 1",
                    "Authorization": f"Bearer {token}", "Host": url.replace("https://", "").split("/")[0],
                    "User-Agent": f"Dalvik/2.1.0 (Linux; U; {ANDROID_OS_VERSION}; {USER_AGENT_MODEL} Build/QKQ1.190825.002)"
                }
                try:
                    async with session.post(url, headers=headers, data=data, ssl=False) as resp:
                        if resp.status == 200:
                            # মেমোরি এবং সময় বাঁচাতে শুধু প্রথম রিকোয়েস্টে প্লেয়ার ইনফো রিড করা হবে
                            if need_info: return True, await resp.read()
                            return True, None
                        return False, None
                except Exception: return False, None

        # টাস্ক জেনারেট করা হচ্ছে, প্রথমটির জন্য need_info=True
        tasks = [
            asyncio.create_task(sem_visit(tokens[i % len(tokens)], need_info=(i == 0)))
...
def send_visits(server, uid):
    ...
    if not tokens: return jsonify({"error": "❌ No valid tokens could be generated. Check accounts."}), 500
    ...
    if player_info:
        ...
        }, sort_keys=False), mimetype='application/json'), 200
    else:
